...
ButtonHandler::ButtonHandler(uint8_t pin)
    : pin(pin), lastButtonState(HIGH), buttonState(HIGH), lastDebounceTime(0),
      lastPressTime(0) {}

void ButtonHandler::init() { pinMode(pin, INPUT_PULLUP); }
...
bool ButtonHandler::isPressed() const { return buttonState == LOW; }
